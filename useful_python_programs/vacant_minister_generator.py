...
def get_tags():
	tag_files = open(r'C:\Users\Marxivi\Documents\Paradox Interactive\Hearts of Iron IV\mod\totmod\common\country_tags\TOT_countries.txt', "r+")
	tags = []
...
def make_blank_chars(tags):
	output_file = open(r'C:\Users\Marxivi\Documents\Paradox Interactive\Hearts of Iron IV\mod\totmod\common\characters\_blank_characters.txt', "w+")
	output_file.write("characters = {\n")

	for tag in tags:
		if not check_folder(f'{tag}_Generic_Vacant_hog = {{', Path(r'C:\Users\Marxivi\Documents\Paradox Interactive\Hearts of Iron IV\mod\totmod\common\characters')):
			text = f"""
		{tag}_Generic_Vacant = {{
			name = minister_Generic_Vacant
			portraits = {{
				civilian = {{ small = "GFX_idea_minister_Generic_Vacant" }}
			}}
			advisor = {{
				slot = head_of_government
				traits = {{ head_of_government }}
				idea_token = {tag}_Generic_Vacant_hog
				ledger = civilian
				cost = -1
				removal_cost = -1
			...
			advisor = {{
				...
				idea_token = {tag}_Generic_Vacant_eco
				ledger = civilian
				cost = -1
				removal_cost = -1
				allowed = {{ original_tag = {tag} }}
			...
			advisor = {{
				...
				idea_token = {tag}_Generic_Vacant_for
				ledger = civilian
				cost = -1
				removal_cost = -1
				allowed = {{ original_tag = {tag} }}
			...
			advisor = {{
				...
				idea_token = {tag}_Generic_Vacant_sec
				ledger = civilian
				cost = -1
				removal_cost = -1
				allowed = {{ original_tag = {tag} }}
...
def recruit_blank_chars(tags):
	history_folder = Path(r'C:\Users\Marxivi\Documents\Paradox Interactive\Hearts of Iron IV\mod\totmod\history\countries')
	for file in history_folder.iterdir():
		flag = False
		for tag in tags:
			text = f"""
# Vacant
recruit_character = {tag}_Generic_Vacant
			...
			if tag == file.name[:3]:
				with file.open('r+', encoding="utf8") as history_file:
					for line in history_file:
						...
						elif f"{tag}_Generic_Vacant" not in line and "# Vacant\n" != line:
							old_file += line
...
def add_slots_as_traits():
	characters_folder = Path(r'C:\Users\Marxivi\Documents\Paradox Interactive\Hearts of Iron IV\mod\totmod\common\characters')
